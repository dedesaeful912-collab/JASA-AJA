...
async function renderCustomer(){const os=await customerOrders();$("dash").innerHTML='<div class="heading"><span class="tag">CUSTOMER</span><h2>Pesanan Saya</h2><p class="muted">Halo, '+esc(currentProfile?.full_name||currentUser.email)+'</p></div><div class="toolbar"><button class="secondary" onclick="scrollToId(\'booking\')">+ Pesan jasa</button><button class="secondary" onclick="renderDashboard()">↻ Refresh</button></div>'+(os.length?os.map(o=>orderCard(o,"customer")).join(""):'<div class="loading">Belum ada pesanan.</div>')}
