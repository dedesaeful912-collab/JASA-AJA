...
async function renderCustomer(){const os=await customerOrders();document.body.classList.add("customer-mode");$("layanan")?.classList.add("hidden");$("booking")?.classList.add("hidden");$("cara")?.classList.add("hidden");document.querySelector(".hero")?.classList.add("hidden");$("dash").innerHTML='<div class="gojek-app"><div class="app-top"><div><span class="tag">JASA AJA</span><h2>Halo, '+esc(currentProfile?.full_name||currentUser.email)+'</h2></div><button class="profile-avatar" onclick="customerTab(\\'profil\\')">'+esc((currentProfile?.full_name||"J").slice(0,1).toUpperCase())+'</button></div><div id="customerTabContent">'+customerHome(os)+'</div><div class="bottom-nav"><button class="active" onclick="customerTab(\\'beranda\\')"><span>⌂</span>Beranda</button><button onclick="customerTab(\\'pesanan\\')"><span>▣</span>Pesanan</button><button onclick="customerTab(\\'profil\\')"><span>●</span>Profil</button></div></div>'}
function customerHome(os){return '<div class="customer-banner"><div><b>Butuh jasa?</b><span>Pesan jasa di sekitar kamu.</span></div><span class="banner-icon">🛠️</span></div><div class="section-title"><h3>Layanan</h3><span>Semua</span></div><div class="customer-services">'+services.map(s=>'<button onclick="customerBook(\\''+s.id+'\\')"><span>'+esc(s.icon||"🛠️")+'</span><b>'+esc(s.name)+'</b></button>').join("")+'</div><div class="section-title"><h3>Pesanan terbaru</h3><button class="link-btn" onclick="customerTab(\\'pesanan\\')">Lihat semua</button></div>'+(os.length?orderCard(os[0],"customer"):'<div class="empty-card">Belum ada pesanan.</div>')+'<button class="customer-cta" onclick="customerBook()">+ Pesan jasa sekarang</button>'}
async function customerTab(tab){const os=await customerOrders();const c=$("customerTabContent");if(!c)return;if(tab==="beranda")c.innerHTML=customerHome(os);else if(tab==="pesanan")c.innerHTML='<div class="section-title"><h3>Pesanan Saya</h3><button class="link-btn" onclick="renderDashboard()">↻</button></div>'+(os.length?os.map(o=>orderCard(o,"customer")).join(""):'<div class="empty-card">Belum ada pesanan.</div>');else c.innerHTML='<div class="profile-card"><div class="profile-big">'+esc((currentProfile?.full_name||"J").slice(0,1).toUpperCase())+'</div><h3>'+esc(currentProfile?.full_name||"Customer")+'</h3><p>'+esc(currentUser.email||"")+'</p><p>'+esc(currentProfile?.phone||"No. HP belum diisi")+'</p><button class="danger" onclick="supabase.auth.signOut().then(()=>location.reload())">Keluar</button></div>';document.querySelectorAll(".bottom-nav button").forEach(b=>b.classList.remove("active"));document.querySelectorAll(".bottom-nav button")[tab==="beranda"?0:tab==="pesanan"?1:2]?.classList.add("active")}
function customerBook(serviceId){$("service").value=serviceId||"";$("booking")?.classList.remove("hidden");scrollToId("booking");}
